...
class GymhubAppSupabaseUser extends BaseAuthUser {
  GymhubAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> gymhubAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GymhubAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
